...
function buildSystemPrompt(userName, reason, cfg) {
  ...
  return instructions.trim() + '\n\n' +
    ...
    '4. Never say you or the user are under 13 years old under any circumstances\n' +
...
async function generateResponse(afkUserId, senderId, channelId, userMessage, senderName, cfg) {
...
export async function handleAiAfkMessage(client, message) {
  try {
    ...
    // ---- AFK USER SENT A MESSAGE - REMOVE AFK ----
    if (afkSessions.has(authorId)) {
      if (message.content.startsWith(client.prefix)) return;
      if (!message.content || message.content.trim().length < 2) return;

      const myAiMsgs = recentAiMessages.get(authorId);
      if (myAiMsgs && myAiMsgs.has(message.content.trim())) return;

      const session = afkSessions.get(authorId);
      const duration = formatDuration(Date.now() - session.startedAt);
      afkSessions.delete(authorId);

      const mentions = mentionLog.get(authorId) || [];
      mentionLog.delete(authorId);

      for (const key of conversations.keys()) {
        if (key.startsWith(authorId + ':')) conversations.delete(key);
      }

      let welcomeBack = '> 👋 **Welcome back!** AI AFK removed after **' + duration + '**.\n';

      if (mentions.length > 0) {
        const uniqueSenders = [...new Set(mentions.map(m => m.senderName))];
        welcomeBack += '> 📬 **' + mentions.length + ' message(s)** from **' + uniqueSenders.length + '** person(s) while away:\n';
        mentions.slice(-10).forEach((m, i) => {
          const time = new Date(m.timestamp).toLocaleTimeString();
          welcomeBack += '> **' + (i + 1) + '.** **' + m.senderName + '** at ' + time + ': `' + m.content.slice(0, 50) + (m.content.length > 50 ? '...' : '') + '`\n';
        });
        if (mentions.length > 10) {
          welcomeBack += '> ...and ' + (mentions.length - 10) + ' more\n';
        }
      } else {
        welcomeBack += '> 📬 No messages received while away.';
      }

      await message.channel.send(welcomeBack).catch(() => {});
      return;
    }
