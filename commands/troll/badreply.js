...
export default {
  name: "badreply",
  description: "Send automatic toxic replies",
    ...
    if (!args.length) {
      return message.channel.send(formatThreeBlock(
        ...
        [
          ...
          ["Stop", `${client.prefix}badreply stop @user`],
          ["List", `${client.prefix}badreply list`]
        ],
        [
          ["Info", "Auto-reply with bad words to a specific user."]
        ]
      ));
    }
  ...
  async startBadReply(client, message, args) {
    ...
    if (message.mentions.users.size > 0) {
      ...
      try {
        ...
        return message.channel.send(formatThreeBlock("Barro Badreply", [["Target", "Unknown"]], [["Result", "User not found."]]));
      }
    }

    if (!targetUser) {
      return message.channel.send(formatThreeBlock("Barro Badreply", [["Target", "Unknown"]], [["Result", `Usage: ${client.prefix}badreply @user`]]));
    }
    if (targetUser.id === message.author.id) {
      return message.channel.send(formatThreeBlock("Barro Badreply", [["Target", targetUser.username]], [["Result", "You can't bad reply to yourself!"]]));
    }
    if (targetUser.bot) {
      return message.channel.send(formatThreeBlock("Barro Badreply", [["Target", targetUser.username]], [["Result", "Cannot bad reply to bot accounts."]]));
    }

    const guildId = message.guild?.id || "dm";
    const sessionKey = `${targetUser.id}:${guildId}`;
    if (badReplySessions.has(sessionKey)) {
      return message.channel.send(formatThreeBlock("Barro Badreply", [["Target", targetUser.username]], [["Result", "That user is already being bad replied to!"]]));
    }

    const task = TaskManager.createTask(`badreply_${targetUser.id}`, guildId);
    if (!task) {
      return message.channel.send(formatThreeBlock("Barro Badreply", [["Target", targetUser.username]], [["Result", "Failed to create bad reply task!"]]));
    }
    ...
    await message.channel.send(formatThreeBlock(
      ...
      [["Result", "Every message they send will receive a mock reply."]]
    ));
  ...
  async stopBadReply(client, message, args) {
    ...
    if (!targetUser) {
      return message.channel.send(formatThreeBlock("Barro Badreply", [["Target", "Unknown"]], [["Result", `Please specify which user to stop bad replying to! ${client.prefix}badreply stop <@user/userID>`]]));
    }
    ...
    if (!sessionData) {
      return message.channel.send(formatThreeBlock("Barro Badreply", [["Target", targetUser.username]], [["Result", "That user is not being bad replied to!"]]));
    }
    ...
    await message.channel.send(formatThreeBlock(
      ...
      [
        ...
        ["Replies", String(sessionData.replyCount)]
      ],
      [["Result", "Badreply stopped."]]
    ));
  },
  ...
  async listActive(client, message) {
    ...
    if (activeSessions.length === 0) {
      return message.channel.send(formatThreeBlock("Barro Badreply", [[style("Sessions", "0;30"), style("0", "37")]], [[style("Result", "0;30"), style("No active bad reply sessions!", "37")]]));
    }

    const sessionRows = activeSessions.map(data => [
      style(data.targetUsername, "37"),
      style(`${this.formatDuration(Date.now() - data.startedAt)} | ${data.replyCount} replies`, "37")
    ]);
    const body = formatAnsiBlocks([
      [formatHeaderTitle('Barro Badreply')],
      [style("Active sessions", "0;30"), ...sessionRows.flatMap(r => [r[0], r[1]])],
      [style("Result", "0;30"), style(`${activeSessions.length} active session(s)`, "37")]
    ]);
